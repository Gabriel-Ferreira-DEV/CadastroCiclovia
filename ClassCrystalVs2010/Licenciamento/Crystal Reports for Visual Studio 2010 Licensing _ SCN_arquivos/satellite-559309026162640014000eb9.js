...
SOCIAL_ORGANIC:["facebook.com","twitter.com","linkedin.com","youtube.com","plus.google.com","slideshare.net","pinterest.com","instagram.com","weibo.com","flickr.com"]
}
};
...
var curDomain=CURRENT_URL.split("?")[0];
var imageURL;
var curDomain = CURRENT_URL.split("//")[1].split("/")[0];
...
var refDomain = domainVal[0];
}

 if(typeof(result.Utils.getCurrentPageParamsKey("lf1")) != "undefined"){
imageURL= p+ "://ad.doubleclick.net/ad/N69702.547841VISUALIQINC/B8550905.115537294;sz=1x1;u=" +result.Utils.getCurrentPageParams()+"-"+encodeURIComponent(CURRENT_URL) +"|"+REFERRER_URL+"|ord=["+result.timestamp +"]";
}
else if(typeof(result.Utils.getCurrentPageParamsKey("source")) != "undefined" ) 
{
	if(result.Utils.getCurrentPageParamsKey("source").substring(0,6).toLowerCase()== 'social')
	{
		imageURL = p+ "://ad.doubleclick.net/ad/N69702.547841VISUALIQINC/B8550905.115537296;sz=1x1;u=" + result.source+"-"+result.Utils.getCurrentPageParams()+"-"+curDomain +"|"+REFERRER_URL+"|ord=["+result.timestamp +"]";
	}
	else if(result.modeOfAccess == "SOCIAL_ORGANIC"){
	imageURL=p+"://ad.doubleclick.net/ad/N69702.547841VISUALIQINC/B8550905.115537517;sz=1x1;u="+result.Utils.getCurrentPageParams()+"-"+curDomain+"|"+REFERRER_URL+"|ord=["+ result.timestamp +"]";
   }
}
else if(result.modeOfAccess == "SOCIAL_ORGANIC"){
	imageURL=p+"://ad.doubleclick.net/ad/N69702.547841VISUALIQINC/B8550905.115537517;sz=1x1;u="+result.Utils.getCurrentPageParams()+"-"+curDomain+"|"+REFERRER_URL+"|ord=["+ result.timestamp +"]";
	...
	imageURL=p+"://ad.doubleclick.net/ad/N69702.547841VISUALIQINC/B8550905.115537295;sz=1x1;u="+result.source+"-"+result.query+"-"+result.Utils.getCurrentPageParams()+"-"+curDomain+"|ord=["+result.timestamp +"]";
}
else if(result.modeOfAccess == "DIRECT"){
	imageURL=p+"://ad.doubleclick.net/ad/N69702.547841VISUALIQINC/B8550905.115537428;sz=1x1;u="+result.Utils.getCurrentPageParams()+"-"+curDomain+"|ord=["+ result.timestamp +"]";
}
else if(refDomain != curDomain && refDomain.toLowerCase().indexOf("sap") < 0){
	imageURL=p+"://ad.doubleclick.net/ad/N69702.547841VISUALIQINC/B8550905.115537427;sz=1x1;u="+result.Utils.getCurrentPageParams()+"-"+curDomain+"|"+referrerDomain+"|ord=["+ result.timestamp +"]";
}
