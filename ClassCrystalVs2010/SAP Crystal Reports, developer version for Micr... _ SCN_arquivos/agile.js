...
(function ($j) {
    ...
    $j.fn.agile_carousel = function (options) {
        ...
        return this.each(function () {
            ...
            $j.each(carousel_data, function (key, value) {
                number_of_slides++;
            });
            ...
            function get_trigger_type(check_me) {
                if (change_on_hover !== "") {
                    ...
                    if ($j.inArray(check_me, change_on_hover_array) != -1) {
            ...
            for (var key in carousel_data) {
                ...
                if (carousel_data.hasOwnProperty(key)) {
                    ...
                    graphic = obj_inner.graphic;
		    title = obj_inner.title;
		    subtitle = obj_inner.subtitle;
		    body = obj_inner.body;
		    linkto = obj_inner.linkto;
                    ...
                    if (graphic) {
                        // render
                        ac_html += "<div class='slide_" + i + " slide' style='border: none; margin: 0; padding: 0; height: " + slide_height + "px; width: " + slide_width + "px;'><div class='slide_inner'><img class='scn-hero' src='" + graphic  +"'>";
			ac_html += "<div class='caption'><h2> "+ title +"</h2><div class='cont-block'>";
			if (subtitle){
			ac_html += "<h3> "+ subtitle +"</h3>";
			}
			ac_html += "<p> "+ body +"</p></div>";
			if(linkto){
			ac_html += "<a href='"+ linkto +"' class='btn-blu' ><span>Learn More</span></a>";
			}
			ac_html += "</div></div></div>";
            ...
            function transition_slides(button_data) {
                ...
                if (ac_disabled !== true) {
                    ...
                    current_slide = $j(ac_slides).eq(current_slide_index);
                    ...
                    next_slide = $j(ac_slides).eq(next_slide_index);
	    ...
	    $j('.numbered_button').live('hover', function(){
		    pause_slideshow();
		});
			...
			////////////////
			 ...
			 $j(agile_carousel_buttons_click).click(function(){
				 pause_slideshow();
				if(obj.find(':animated').length < 1){
					transition_slides($j(this).data().options);
				} else {
					$jthis = $j(this);
					// don't use timer on next & previous buttons... causes strage, infinite cycle
					if($jthis.data("options").button_action != "next" && $jthis.data("options").button_action != "previous") {
							function check_transition(){
									
									if(ac_slides_container.find(':animated').length < 1){
										transition_slides($jthis.data().options);
										clearInterval($jthis.data("options").timeout);
									} // if
								} // function
								
							t = setInterval(check_transition,30);	
								
							$jthis.data("options").timeout = t;
					}// if
				} // else
			...
			$j(agile_carousel_buttons_hover).hover(function() {
				pause_slideshow();
    			if(ac_slides_container.find(':animated').length < 1){
					transition_slides($j(this).data().options);
		...
		pause_button.removeClass("play_button");
            ...
            pause_button.click(function () {
                var $jthis = $j(this);
                if ($jthis.data("options").paused === true) {
                    play_slideshow();

                } else if ($jthis.data("options").paused === false) {
                    pause_slideshow();
                    clearInterval(ac_timer);
                }
            }); // click
            ...
            $j('.hover_previous_next_button').hover(function() {
                $j(this).find(".hover_previous_next_button_inner").stop().fadeTo("fast", 0.85);
            },
            // function
            function() {
                $j(this).find(".hover_previous_next_button_inner").stop().fadeTo("fast", 0.00);
            }); // hover
